...
## Case 1: Explain Y with a linear regression


Let's consider the following model $\pi(x) = \beta_0 + \beta_1 x$ :

$$\mathbb{P}(Y=1|x)=\pi (x) = \beta_0 + \beta_1 x$$

Problem: $\pi(x)$ takes values oustide $[0,1]$ interval. 

## Case 2: Explain Y with a logistic regression

We introduce the *logit* function
...
## Univariate approach: Shurely Independant Screening
...
# Predict the histology on the test set
...
You con control overfitting by using a cross-validation approach on the dataset `data train`
