...
alternative_programs = list(
  ...
  program_1best_reglog = function(data_train, data_test) {
  ...
  program_2best_reglog = function(data_train, data_test) {
    ...
    #plot(ms["beta",], -log10(ms["pval",]))
    g = colnames(ms[,order(ms["pval",])])[1:2]
...
generate_submission_files = function(program, data_train, data_test, prefix) {
  if (missing(prefix)) {
    prefix = format(x = Sys.time( ), format = "%Y-%m-%d_%Hh%Mm%Ss")
  }
